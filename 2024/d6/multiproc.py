...
def test_position(args):
    map_data, pos, guard_base_position = args
    map_copy = [row[:] for row in map_data]
    return travel_map_simulation_infinite(map_copy, pos, guard_base_position)
...
def main():
    ...
    position_to_test = [
        (i,j) for i in range(len(map)) for j in range(len(map[i])) if map[i][j] == "."
    ]

    args = [(map, pos, guard_base_position) for pos in position_to_test]

    with Pool(processes=16) as pool:
        results = pool.map(test_position, args)

    res = sum(1 for x in results if x)
    print(res)
